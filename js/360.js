...
function init() {
  ...
  var myTxt = new createjs.Text("360", '24px Ubuntu', "#dd2958");
  myTxt.x = (stage.canvas.width / 2);
  myTxt.y = (stage.canvas.height / 2) - 30;
  myTxt.alpha = 1;
  ...
  function img360Loaded(event) {
    loaded++;
    bg.graphics.clear();
    const k = loaded / totalFrames;
    if (k == 1) {
      myTxt.alpha = 0;
      // bg.graphics.beginFill("#dd2958").drawRect(0, 0, (stage.canvas.width * loaded) / totalFrames, stage.canvas.height);
    } else { 
      myTxt.text = `${Math.round(k * 100)}%`;
      bg.graphics.beginFill("#dd2958").drawRect(10, (stage.canvas.height/2)-1, ((stage.canvas.width - 20) * loaded) / totalFrames,  2,);
    }
    bg.graphics.endFill();
